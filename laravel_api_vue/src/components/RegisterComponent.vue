...
import { useAuthStotre } from '@/stores/auth';
const formData = reactive({
email:'',
password:'',
name:'',
})
const authStore = useAuthStotre();
</script>
<template>
  <form @submit.prevent="()=>{
    ...
    <!-- email -->
    <div class="mb-5">
      <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your mail</label>
  ...
  <div class="mb-5">
    ...
    <input v-model="formData.name" type="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />

  </div>
  <div class="flex items-end mb-5">

    <span  class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">already have an account?
